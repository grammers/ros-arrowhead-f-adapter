...
void SensorHandler::initSensorHandler(std::string baseName, f_void_f callback){
	...
	// callback function
	SensorHandler::callback = callback;
...
bool SensorHandler::registerSensor(){
	printf("\nREGISTRATION (%s, %s)\n\n", config.SECURE_THIS_INTERFACE ? "Secure Provider" : "Insecure Provider", config.SECURE_ARROWHEAD_INTERFACE ? "Secure AHInterface" : "Insecure AHInterface");

	
	///////////
	// HTTPS //
	///////////
	if(config.SECURE_THIS_INTERFACE){
...
--
*/
int SensorHandler::Callback_Serve_HTTP_POST(const char *url, const char *payload){
	printf("POST received in SensorHandler.\npayload: %s\nurl: %s\n", payload, url);
