...
void SensorHandler::processConsumer(std::string pJsonSenML, bool _bSecureArrowheadInterface, std::string msg) {
	json_object *obj = json_tokener_parse(pJsonSenML.c_str());
	
	SensorHandler::msg = msg;
...
//
	...
	sInterface = string(json_object_get_string(jIntf0));
     ...
     if(bSecureProviderInterface){
		...
		printf("sProvierURI (sensor handler): %s\n", sProviderURI.c_str());
...
void SensorHandler::sendRequestToProvider(std::string _sProviderURI){
	...
	curl_global_init(CURL_GLOBAL_ALL);
  	...
  	if(curl){
		...
		if(res != CURLE_OK)
			fprintf(stderr, "curl_easy_perform() failed (in sensorHandler): %s\n", curl_easy_strerror(res));
...
void SensorHandler::sendHttpsRequestToProvider(std::string _sProviderURI){
	...
	curl_global_init(CURL_GLOBAL_ALL);
  	...
  	if(curl){
		...
		if(res != CURLE_OK)
			fprintf(stderr, "curl_easy_perform() failed (in https in sensorHandler): %s\n", curl_easy_strerror(res));
