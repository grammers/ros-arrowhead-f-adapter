...
int main(int argc, char **argv)
{
	...
	ros::Publisher rescived_pub = n.advertise<sensor_msgs::Temperature>("temperature_consumer", 10);
	
	//prams
	ros::NodeHandle nh("~");
	// service
	nh.param<std::string>("SERVICE_DEFINITION_CONSUMER", consumer.config.SERVICE_DEFINITION,"IndoorTemperature_providerExample");
	nh.param<std::string>("UNIT_CONSUMER", consumer.config.UNIT, "NOT SPIFFED");	
	nh.param<std::string>("SERVICE_DEFINITION_PUBLISHER", publisher.config.SERVICE_DEFINITION,"IndoorTemperature_providerExample");
	nh.param<std::string>("UNIT_PUBLISHER", publisher.config.UNIT, "NOT SPIFFED");	
	// target
	nh.param<std::string>("TARGET_SYSTEM_NAME_CONSUMER", consumer.config.TARGET_SYSTEM_NAME, "SecureTemperatureSensor");
	nh.param<std::string>("TARGET_ADDRESS_CONSUMER", consumer.config.TARGET_ADDRESS, "10.10.10.122");
	nh.param<int>("TARGET_PORT_CONSUMER", consumer.config.TARGET_PORT, 8452);
	nh.param<std::string>("TARGET_SYSTEM_NAME_PUBLISHER", publisher.config.TARGET_SYSTEM_NAME, "SecureTemperatureSensor");
	nh.param<std::string>("TARGET_ADDRESS_PUBLISHER", publisher.config.TARGET_ADDRESS, "10.10.10.122");
	...
	nh.param<std::string>("THIS_ADDRESS", consumer.config.THIS_ADDRESS, "10.0.0.40");
	nh.param<std::string>("THIS_ADDRESS6", consumer.config.THIS_ADDRESS6, "[fe80::1a57:58d9:c43:6319]");
	nh.param<std::string>("THIS_SYSTEM_NAME", consumer.config.THIS_SYSTEM_NAME, "client1");
	// service some
	nh.param<std::string>("INTERFACE", consumer.config.INTERFACE, "REST-JSON-SENML");
	nh.param<std::string>("SECURITY", consumer.config.SECURITY, "toke");
	nh.param<bool>("SECURE_PROVIDER_INTERFACE", consumer.config.SECURE_PROVIDER_INTERFACE , false);
	nh.param<bool>("SECURE_ARROWHEAD_INTERFACE", consumer.config.SECURE_ARROWHEAD_INTERFACE , false);
	// core some
	nh.param<std::string>("ACCESS_URI", consumer.config.ACCESS_URI ,"http://arrowhead.tmit.bme.hu:8440/orchestrator/orchestration");
	nh.param<std::string>("ACCESS_URI_HTTPS", consumer.config.ACCESS_URI_HTTPS ,"https://arrowhead.tmit.bme.hu:8441/orchestrator/orchestration");
	// orchestrator
	...
	nh.param<bool>("EXTERNAL_SERVICE_REQUEST", consumer.config.EXTERNAL_SERVICE_REQUEST, false);
	...
	publisher.config.SECURE_PROVIDER_INTERFACE = consumer.config.SECURE_PROVIDER_INTERFACE;
	publisher.config.SECURE_ARROWHEAD_INTERFACE = consumer.config.SECURE_ARROWHEAD_INTERFACE;
	...
	publisher.config.EXTERNAL_SERVICE_REQUEST = consumer.config.EXTERNAL_SERVICE_REQUEST;
	...
	convert.init("sensor_id", publisher.config.UNIT, publisher.config.THIS_SYSTEM_NAME);

	publisher.init_publisher();
	consumer.init_consumer(Converter::parce);
	...
	while(ros::ok())
	{
		...
		consumer.sendGETRequest();

		// arrowhead ask provider for data
    	publisher.sendPOSTRequest(convert.getJsonMsgs());
